...
class FileController {
  ...
  async getAllFiles(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await fileService.getAllFilesByUser(userId, parseInt(page as string), parseInt(limit as string))
   ...
   * Lấy danh sách files theo subjectId
   */
  async getFilesBySubjectId(req: Request, res: Response, next: NextFunction) {
    try {
      const userId = req.user?.userId
